...
namespace Ninesky.Controllers
{
    public class UserController : Controller
    {
        ...
        public ActionResult Login(UserLogin login)
        {
            ...
            //验证账号密码
            userRsy = new UserRepository();
            if (userRsy.Authentication(login.UserName, Common.Sha256(login.Password)) == 0)
            {
        ...
        {
            userRsy = new UserRepository();
            if (userRsy.Authentication(UserName, Common.Sha256(userChangePassword.Password)) == 0)
            {
                var _user = userRsy.Find(UserName);
                if (_user == null)
                {
                ...
                _user.Password = Common.Sha256(userChangePassword.NewPassword);
        ...
        {
            userRsy = new UserRepository();
            if(userRsy.Authentication(UserName,Ninesky.Common.Sha256(user.Password))==0)
            {
                var _user = userRsy.Find(UserName);
        ...
        public ActionResult VerificationCode()
        {
            ...
            //获取验证码
            string _verificationText = Common.VerificationText(_verificationLength);
            //存储验证码
